...
protected:
	void SetShaderLight(GLuint* buffer, const int& numLights, const Light* l);
	void SetShaderMaterial(GLuint* buffer, const int& numObjects, const int& totalNumMats, const int* numMaterials, Material** m);

	virtual void	Resize(int x, int y);	
	void			UpdateShaderMatrices();
	void			BindShader(Shader*s);
	void			SetTextureRepeating(GLuint target, bool state);
	void			SetTextureAnistropicFiltering(GLuint target, GLfloat value);
