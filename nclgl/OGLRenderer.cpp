...
void OGLRenderer::SetShaderLight(GLuint* buffer, const int& numLights, const Light* l) {
	int binding = 1;
	glBindBuffer(GL_UNIFORM_BUFFER, *buffer);
	glBufferData(GL_UNIFORM_BUFFER, sizeof(Light) * numLights, l, GL_DYNAMIC_DRAW);
	glBindBufferBase(GL_UNIFORM_BUFFER, binding, *buffer);

	int ubo = glGetUniformBlockIndex(currentShader->GetProgram(), "Lights");
	glUniformBlockBinding(currentShader->GetProgram(), ubo, binding);
}

void OGLRenderer::SetShaderMaterial(GLuint* buffer, const int& numObjects, const int& totalNumMats, const int* numMaterials, Material** m) {
	int binding = 2;
	glBindBuffer(GL_UNIFORM_BUFFER, *buffer);

	glBufferData(GL_UNIFORM_BUFFER, sizeof(Material) * totalNumMats, NULL, GL_DYNAMIC_DRAW);
	int offset = 0;
	for (int i = 0; i < numObjects; i++) {
		glBufferSubData(GL_UNIFORM_BUFFER, offset, sizeof(Material) * numMaterials[i], m[i]);
		offset += (sizeof(Material) * numMaterials[i]);
	}

	glBindBufferBase(GL_UNIFORM_BUFFER, binding, *buffer);

	int ubo = glGetUniformBlockIndex(currentShader->GetProgram(), "Materials");
	glUniformBlockBinding(currentShader->GetProgram(), ubo, binding);
}
...
void OGLRenderer::SetTextureAnistropicFiltering(GLuint target, GLfloat value) {
