...
Mesh* Mesh::GeneratePlane() {
	...
	for (int i = 0; i < m->numVertices; i++) {
		...
		m->vertices[i++] = origin + Vector3(1, 0, 0);
		m->textureCoords[i] = Vector2(1, 0);

		m->vertices[i++] = origin + Vector3(0, 0, 1);
		m->textureCoords[i] = Vector2(0, 1);
		...
		m->vertices[i++] = origin + Vector3(1, 0, 1);
		m->textureCoords[i] = Vector2(1, 1);
	}
...
Mesh* Mesh::GenerateCustomQuad(const Vector3& origin, const float& width, const float& height) {
	Mesh* m = new Mesh();
	m->type = GL_TRIANGLE_STRIP;
	m->numVertices = 4;
	...
	m->vertices = new Vector3[m->numVertices];
	m->vertices[0] = origin;
	m->vertices[1] = origin + Vector3(width, 0.0f, 0.0f);
	m->vertices[2] = origin + Vector3(0.0f, height, 0.0f);
	m->vertices[3] = origin + Vector3(width, height, 0.0f);
